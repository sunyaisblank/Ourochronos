...
mod suite {
    ...
    fn test_divergence() {
        ...
        let result = TimeLoop::new(Config {
            max_epochs: 50,
            ..default_config()
        })
        .expect("valid configuration")
    ...
    fn test_witness_pattern_primality() {
        let program = parse("0 ORACLE DUP 3 EQ IF { DUP 0 PROPHECY } ELSE { 3 0 PROPHECY }");
        let result = TimeLoop::new(default_config())
            .expect("valid configuration")
            .run(&program);
    ...
    fn test_action_guided_finds_fixed_point() {
        ...
        let program = parse("0 ORACLE 0 PROPHECY");
        let result = TimeLoop::new(action_config())
            .expect("valid configuration")
            .run(&program);
        assert!(matches!(result, ConvergenceStatus::Consistent { .. }));
    }
    ...
    fn test_action_guided_prefers_output() {
        ...
        let program =
            parse("0 ORACLE DUP 0 EQ NOT IF { DUP OUTPUT DUP 0 PROPHECY } ELSE { 1 0 PROPHECY }");
        let result = TimeLoop::new(action_config())
            .expect("valid configuration")
            .run(&program);
...
mod temporal_op_count {
    ...
    fn counts_oracle_and_prophecy_across_body_quotes_and_branches() {
        let program =
            parse("0 ORACLE 0 PROPHECY [ 1 ORACLE POP ] EXEC 1 IF { 2 ORACLE POP }").unwrap();
        ...
        stmts.iter().any(|stmt| match stmt {
            ...
            Stmt::If {
                ...
                contains_op(then_branch, expect)
                    || else_branch
                        .as_deref()
                        .is_some_and(|e| contains_op(e, expect))
            }
            ...
            match parser.parse_program() {
                Ok(program) => {
                    if !contains_op(&program.body, op) {
                        failures.push(format!("{} parsed but produced no Op({:?})", op.name(), op));
                    }
                }
